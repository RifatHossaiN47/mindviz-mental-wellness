...
class LoadingScreen(QWidget):
    ...
    def setup_ui(self):
        ...
        if self.game_data is not None:
            subtitle_text = "Understanding your gameplay patterns"
        elif self.face_image:
            subtitle_text = "Analyzing text and face cues"
        else:
            subtitle_text = "Understanding your feelings"
    ...
    def animate_dots(self):
        ...
        if self.game_data is not None:
            ...
            self.subtitle.setText(f"Analyzing text and face cues{dots}")
        else:
    ...
    def update_progress(self):
        if self.progress < 85:
            ...
            if self.progress < 30:
                ...
                if self.face_image:
                    self.status_label.setText("Analyzing face and text signals...")
                else:
    ...
    def send_text_to_backend(self):
        try:
            ...
            if not isinstance(result, dict) or 'metrics' not in result:
                detail = result.get('detail', 'Invalid analysis response') if isinstance(result, dict) else 'Invalid analysis response'
                self.show_error(str(detail))
                return
            ...
            if result.get('metrics', {}).get('face_signal', {}).get('used'):
                face_signal = result['metrics']['face_signal']
                print(f"[FRONTEND] Face signal: {face_signal.get('emotion')} (conf={face_signal.get('confidence', 0):.2f})")
